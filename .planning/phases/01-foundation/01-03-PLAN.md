...
Use pathlib.Path for all file paths. Fixtures should use `@pytest.fixture` decorator.
  ...
  <verify>
Run: `python -c "import pandas as pd; df = pd.read_csv('tests/fixtures/valid_portfolio.csv'); print(f'Rows: {len(df)}, Columns: {list(df.columns)}')"`
...
1. `pytest tests/` shows all tests passing
