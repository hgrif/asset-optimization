...
must_haves:
  truths:
    - "Package is installable in editable mode via uv"
...
Use the exact structure from RESEARCH.md "Minimal pyproject.toml for Pip Installation" section.
  ...
  <verify>
Run: `uv pip install -e . && uv run python -c "import asset_optimization; print(asset_optimization.__version__)"`
Expected: Prints "0.1.0" without errors.
  </verify>
  <done>Package installs via uv in editable mode and version is accessible.</done>
</task>
...
3. Update `src/asset_optimization/__init__.py`:
  ...
  <verify>
Run: `uv run python -c "from asset_optimization import ValidationError; from asset_optimization.schema import portfolio_schema; print('OK')"`
...
2. `uv run python -c "import asset_optimization"` works
3. `uv run python -c "from asset_optimization import ValidationError"` works
4. `uv run python -c "from asset_optimization.schema import portfolio_schema"` works
