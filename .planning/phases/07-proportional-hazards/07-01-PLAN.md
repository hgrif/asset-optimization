...
must_haves:
  truths:
    - "Assets with higher covariate values show proportionally higher failure rates than the baseline model alone"
    - "Covariate effects are correctly reflected in failure rate and probability outputs"
    - "Missing covariate columns fall back to baseline-only behavior (no errors)"
