...
**Scope note:** Optimizer integration (covariate-aware risk_after) is deferred to a future phase per research recommendation. Optimizer.fit() works with ProportionalHazardsModel via property delegation (params, type_column) but uses baseline-only risk_after for intervention ranking. This is a known, tested limitation documented in the code.
</objective>
...
Note: This plan modifies files created by 07-01. Preserve all existing content from 07-01 when adding new methods/tests.
</execution_context>
...
Also add a comment in the module docstring or class docstring noting:
```python
# Note: Optimizer uses model.params and model.type_column via delegation.
# Optimizer.risk_after calculation uses baseline-only Weibull CDF (does not
# incorporate covariate effects). This is a known limitation - covariate-aware
# optimization is deferred to a future phase.
```
  </action>
  <verify>
```bash
...
<task type="auto">
  ...
  <action>
Add new test classes to tests/test_proportional_hazards.py (append to existing file from 07-01):
...
class TestProportionalHazardsSimulatorIntegration:
    ...
    def test_simulator_runs_with_ph_model(self, simulation_config, portfolio_with_covariates):
        ...
        assert result.total_cost() >= 0  # Note: method call, not attribute
        assert len(result.summary) == 5  # 5 years of summary data
    ...
    def test_covariate_affects_failure_rate_in_simulation(self, simulation_config, portfolio_with_covariates):
        ...
        # With positive coefficient, expect more failures (higher total cost)
        # Note: stochastic, but with seed=42 and strong coefficient, should be consistent
        assert result_high.total_cost() >= result_baseline.total_cost() * 0.5  # At least not way lower
...
class TestProportionalHazardsOptimizerCompat:
    """Test that Optimizer works with ProportionalHazardsModel via delegation.

    Note: Optimizer uses baseline-only risk_after calculation (does not incorporate
    covariate effects). This is a known limitation documented in the code. These
    tests verify that Optimizer.fit() completes without error when given a
    ProportionalHazardsModel.
    """

    def test_optimizer_fit_completes_with_ph_model(self):
        """Optimizer.fit() runs without error using ProportionalHazardsModel."""
        from asset_optimization import Optimizer, SimulationConfig, WeibullModel
        from asset_optimization.models import ProportionalHazardsModel
        from asset_optimization.portfolio import validate_portfolio
        ...
        portfolio = validate_portfolio(pd.DataFrame({
            'asset_id': ['P1', 'P2', 'P3'],
            'asset_type': ['pipe'] * 3,
            'material': ['PVC'] * 3,
        ...
        optimizer = Optimizer(ph, config)

        # This should complete without error
        # Note: risk_after uses baseline-only Weibull CDF (known limitation)
        result = optimizer.fit(portfolio, budget=10000)

        assert result is not None
        # Optimizer should produce some recommendations
        assert hasattr(result, 'recommended_interventions') or hasattr(result, 'schedule')
```
  </action>
  <verify>
```bash
cd /Users/henkgriffioen/code/asset-optimization
pytest tests/test_proportional_hazards.py -v -k "Integration or ConditionalProbability or OptimizerCompat"
```
  </verify>
  <done>
Simulator integration tests pass. Optimizer compatibility test passes. ProportionalHazardsModel works correctly with Simulator.run() and Optimizer.fit().
  </done>
</task>
...
- Optimizer.fit() works with ProportionalHazardsModel (via delegation, baseline-only risk_after)
- Covariate effects are correctly applied during simulation
- All tests pass including simulation integration tests and optimizer compatibility test
- ProportionalHazardsModel importable from main package
- Optimizer limitation documented in code comments
</success_criteria>
