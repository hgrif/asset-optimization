...
must_haves:
  truths:
    ...
    - "Reconstruct intervention has upgrade_type set (intended surface type change; simulator does not apply this yet)"
...
2. `TestRoadDomainDefaultInterventions`:
   ...
   - Test encode_covariates returns a copy (input DataFrame unchanged)
...
- `validate(df)`: use the road Pandera schema, catch SchemaError -> ValidationError (same pattern as PipeDomain)
  ...
  - Note: `upgrade_type` is metadata only right now; the Simulator does not apply it to change surface types.
- `default_model()`: return ProportionalHazardsModel(baseline=WeibullModel(road_params, type_column="surface_type"), covariates=["traffic_load_encoded", "climate_zone_encoded"], coefficients={"traffic_load_encoded": 0.35, "climate_zone_encoded": 0.15})
  - Add a static method `encode_covariates(df)` that takes a road DataFrame, returns a copy, and adds numeric columns `traffic_load_encoded` (low=1, medium=2, high=3) and `climate_zone_encoded` (temperate=1, cold=2, hot_dry=3, hot_humid=4) for use with ProportionalHazardsModel
