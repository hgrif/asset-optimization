...
- from asset_optimization import Portfolio, WeibullModel
  ...
  <verify>
pytest tests/conftest.py --collect-only 2>&1 | grep -E "(optimization_portfolio|weibull_model)"
  </verify>
  <done>
...
class TestGreedyAlgorithm:
    ...
    def test_tie_breaking_oldest_first(self):
...
<success_criteria>
- All test classes organized by concern (Init, Fit, Budget, Greedy, Threshold, Exclusions, Result, EdgeCases)
