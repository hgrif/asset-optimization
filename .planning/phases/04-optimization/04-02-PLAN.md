...
CRITICAL ORDERING: Age must be computed BEFORE calling model.transform() because transform() validates 'age' exists.

Stage 1 - Best intervention per asset:
1. Copy portfolio.data to working DataFrame
2. FIRST: Ensure 'age' column exists (compute from install_date if needed) - this MUST happen before transform()
3. THEN: Call model.transform() to add failure_probability
4. Apply exclusion filter (skip excluded asset_ids)
5. For each asset, compute risk_after for each intervention (Replace, Repair, Inspect)
   - Use weibull_min.cdf(new_age, c=shape, scale=scale) with model.params
6. Compute cost_effectiveness = (risk_before - risk_after) / cost for non-DoNothing
7. Pick intervention with highest cost_effectiveness per asset
8. Note: Inspect has cost_effectiveness=0 (no risk reduction in v1), will only be selected as budget fallback
...
# Add age column BEFORE transform (required by WeibullModel.transform)
...
3. MILP raises NotImplementedError: `python -c "from asset_optimization import Optimizer, Portfolio; import pandas as pd; p = Portfolio.from_dataframe(pd.DataFrame({'asset_id':['A'], 'material':['PVC'], 'install_date':pd.to_datetime(['2000-01-01']), 'condition':[3], 'diameter_mm':[100], 'length_m':[100], 'location':['Z']})); Optimizer('milp').fit(p, None, 1000)"`
4. Unknown strategy raises ValueError: `python -c "from asset_optimization import Optimizer, Portfolio; import pandas as pd; p = Portfolio.from_dataframe(pd.DataFrame({'asset_id':['A'], 'material':['PVC'], 'install_date':pd.to_datetime(['2000-01-01']), 'condition':[3], 'diameter_mm':[100], 'length_m':[100], 'location':['Z']})); Optimizer('unknown').fit(p, None, 1000)"`
...
- Age is computed BEFORE model.transform() call (critical ordering)
