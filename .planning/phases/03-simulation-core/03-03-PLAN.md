...
must_haves:
  truths:
    ...
    - "Older assets fail more frequently than younger assets in simulation results"
    ...
    - "User can retrieve available intervention options per asset at any timestep"
  artifacts:
    - path: "src/asset_optimization/simulation/simulator.py"
      provides: "Simulator class with run() and get_intervention_options() methods"
  ...
  key_links:
    - from: "src/asset_optimization/simulation/simulator.py"
      ...
      via: "DeteriorationModel.params for type-specific Weibull parameters"
      pattern: "self\\.model\\.params\\["
    - from: "src/asset_optimization/simulation/simulator.py"
      to: "scipy.stats.weibull_min"
      via: "Survival function for conditional probability"
...
1. `__init__(self, deterioration_model, config, interventions=None)`:
   ...
   - Store interventions dict (default to DO_NOTHING, INSPECT, REPAIR, REPLACE)
...
4. `_calculate_conditional_probability(self, state) -> np.ndarray`:
   ...
   - Access model.params[asset_type] to get (shape, scale) per asset type
   - Calculate S(t) and S(t+1) per asset type
   - Conditional prob = (S(t) - S(t+1)) / S(t)
   - Handle S(t) == 0 case (return 0.0)
   - Note: We access model.params directly rather than model.transform() because transform() returns cumulative probability F(t), not conditional probability. For simulation we need P(fail this year | survived to now).

5. `get_intervention_options(self, state, year) -> pd.DataFrame`:
   - Generate available intervention options for each asset at the given timestep
   - Returns DataFrame with columns: asset_id, intervention_type, cost, age_effect_description
   - For each asset, lists all available interventions from self.interventions dict
   - This method exposes what interventions *could* be done (Phase 4 optimization will select)
   - Example output:
     ```
     asset_id    intervention_type  cost      age_effect
     PIPE-001    DoNothing          0.0       no change
     PIPE-001    Inspect            500.0     no change
     PIPE-001    Repair             5000.0    age - 5
     PIPE-001    Replace            50000.0   age = 0
     PIPE-002    DoNothing          0.0       no change
     ...
     ```
...
# Test get_intervention_options (INTV-04)
state = portfolio.data.copy()
state['age'] = 26.0  # Simulate aged assets
options = sim.get_intervention_options(state, 2026)
print(f'\\nIntervention options generated: {len(options)} rows')
print(f'Unique interventions: {options[\"intervention_type\"].unique().tolist()}')
print(options.head(8))
"
```
  </verify>
  <done>Simulator.run() executes multi-timestep simulation with failure sampling and cost tracking. get_intervention_options() generates available interventions per asset.</done>
</task>
...
- Conditional probability calculated correctly using survival functions (accessing model.params)
...
- get_intervention_options() generates all available interventions per asset per timestep (INTV-04)
</success_criteria>
