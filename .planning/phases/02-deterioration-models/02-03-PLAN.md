...
must_haves:
  truths:
    - "Test suite covers DeteriorationModel ABC interface"
    - "Test suite covers WeibullModel instantiation and validation"
    - "Test suite covers transform() output and immutability"
    - "Test suite verifies performance (<1 second for 1000+ assets)"
    - "Test suite verifies failure rate increases with age for shape > 1"
    - "All tests pass"
  artifacts:
    - path: "tests/test_deterioration.py"
      provides: "Comprehensive test suite for deterioration models"
      min_lines: 150
  key_links:
    - from: "tests/test_deterioration.py"
      to: "src/asset_optimization/models/weibull.py"
      via: "import and instantiation"
      pattern: "from asset_optimization import WeibullModel"
    - from: "tests/test_deterioration.py"
      to: "src/asset_optimization/models/base.py"
      via: "import for ABC testing"
      pattern: "from asset_optimization.models import DeteriorationModel"
---

<objective>
Create comprehensive test suite for deterioration models covering the ABC interface, WeibullModel implementation, and performance requirements.

Purpose: Verify all Phase 2 success criteria are met, especially efficient calculation for 1000+ pipes (<1 second) and correct mathematical behavior.

Output: Test file with passing tests for ABC, WeibullModel validation, transform output, immutability, and performance.
</objective>
...
@.planning/phases/02-deterioration-models/02-02-SUMMARY.md
...
<task type="auto">
  <name>Task 1: Create deterioration model test suite</name>
  <files>tests/test_deterioration.py</files>
  <action>
Create comprehensive test suite following the established test patterns from Phase 1.

**Create `tests/test_deterioration.py`:**
...
class TestWeibullModelMathematical:
    ...
    def test_failure_rate_matches_weibull_formula(self):
        ...
        # Expected hazard: h(t) = (k/λ) * (t/λ)^(k-1)
...
```
  ...
  <verify>uv run pytest tests/test_deterioration.py -v --tb=short</verify>
  <done>All deterioration model tests pass</done>
</task>
...
1. All new deterioration tests pass
2. All existing Phase 1 tests still pass
...
Expected: All tests pass (Phase 1 tests + new deterioration tests).
  </action>
  <verify>uv run pytest tests/ -v --tb=short | tail -20</verify>
...
# 3. Performance test explicitly
uv run pytest tests/test_deterioration.py::TestWeibullModelPerformance -v

# 4. Coverage of success criteria
echo "=== Phase 2 Success Criteria ==="
echo "1. Weibull 2-parameter model: Tested in TestWeibullModelMathematical"
echo "2. Parameters per asset type: Tested in TestWeibullModelMultipleTypes"
echo "3. Vectorized <1 second: Tested in TestWeibullModelPerformance"
echo "4. Pluggable interface: Tested in TestDeteriorationModelInterface"
```
</verification>

<success_criteria>
1. All tests in test_deterioration.py pass
2. All existing Phase 1 tests still pass
3. Performance test confirms <1 second for 1000+ assets
4. Mathematical tests verify Weibull formula correctness
5. ABC interface tests verify pluggable architecture
6. Validation tests verify error handling
7. Immutability tests verify transform() returns copy
</success_criteria>

<output>
After completion, create `.planning/phases/02-deterioration-models/02-03-SUMMARY.md`
</output>
